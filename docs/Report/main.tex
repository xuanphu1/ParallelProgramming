...
\subsection{SIMD Vectorization với AVX-256}

\subsubsection{Nguyên lý hoạt động}

SIMD (Single Instruction Multiple Data) là kỹ thuật cho phép thực hiện cùng một phép toán trên nhiều phần tử dữ liệu đồng thời trong một chu kỳ xung nhịp. AVX-256 (Advanced Vector Extensions 256-bit) là tập lệnh mở rộng của Intel cho phép xử lý 256-bit dữ liệu cùng lúc.

\textbf{Đặc điểm AVX-256:}
\begin{itemize}
    \item Độ rộng thanh ghi: 256 bit
    \item Xử lý được: 8 giá trị float (32-bit) hoặc 4 giá trị double (64-bit) cùng lúc
    \item Yêu cầu CPU: Intel Sandy Bridge trở lên, AMD Bulldozer trở lên
    \item Tăng tốc lý thuyết: Tối đa 8x so với scalar code (với float)
\end{itemize}

\subsubsection{Triển khai Sobel với AVX-256}

Trong đề tài, Sobel filter được triển khai với AVX-256 intrinsics để xử lý 8 pixels cùng lúc:

\textbf{Bước 1: Load dữ liệu}
\begin{itemize}
    \item Sử dụng \texttt{\_mm256\_loadu\_si256} hoặc \texttt{\_mm\_loadu\_si128} để load 16 pixels (128-bit) từ mỗi dòng ảnh
    \item Convert từ 8-bit unsigned char sang 16-bit integer bằng \texttt{\_mm256\_cvtepu8\_epi16}
    \item Tiếp tục convert sang 32-bit integer và cuối cùng sang float để tính toán chính xác
\end{itemize}

\textbf{Bước 2: Tính gradient}
\begin{itemize}
    \item Gradient X: Áp dụng kernel Sobel Gx cho 8 pixels cùng lúc:
    \begin{equation}
    G_x = -P_{00} - 2P_{10} - P_{20} + P_{02} + 2P_{12} + P_{22}
    \end{equation}
    \item Gradient Y: Áp dụng kernel Sobel Gy cho 8 pixels cùng lúc:
    \begin{equation}
    G_y = -P_{00} - 2P_{01} - P_{02} + P_{20} + 2P_{21} + P_{22}
    \end{equation}
    \item Sử dụng các phép toán vector: \texttt{\_mm256\_add\_ps}, \texttt{\_mm256\_sub\_ps}, \texttt{\_mm256\_mul\_ps}
\end{itemize}

\textbf{Bước 3: Tính magnitude}
\begin{itemize}
    \item Magnitude được tính bằng: $M = \sqrt{G_x^2 + G_y^2}$
    \item Sử dụng \texttt{\_mm256\_sqrt\_ps} để tính căn bậc hai cho 8 giá trị cùng lúc
    \item Clamp giá trị về [0, 255] bằng \texttt{\_mm256\_min\_ps}
\end{itemize}

\textbf{Bước 4: Store kết quả}
\begin{itemize}
    \item Convert từ float về integer: \texttt{\_mm256\_cvtps\_epi32}
    \item Pack từ 32-bit về 16-bit: \texttt{\_mm\_packus\_epi32}
    \item Pack từ 16-bit về 8-bit: \texttt{\_mm\_packus\_epi16}
    \item Store 8 pixels vào bộ nhớ: \texttt{\_mm\_store\_epi64}
\end{itemize}

\textbf{Kết hợp với OpenMP:}
\begin{itemize}
    \item Sử dụng \texttt{\#pragma omp parallel for} để chia ảnh theo dòng
    \item Mỗi thread xử lý một nhóm dòng độc lập
    \item SIMD xử lý 8 pixels trong mỗi dòng, OpenMP xử lý nhiều dòng song song
    \item Kết quả: Song song hóa 2 tầng (dòng + pixel)
\end{itemize}

\subsubsection{Ưu điểm và hạn chế của SIMD}

\textbf{Ưu điểm:}
\begin{itemize}
    \item Không cần GPU, hoạt động trên mọi CPU hỗ trợ AVX2
    \item Tăng tốc đáng kể (3-4x) so với scalar code
    \item Overhead thấp, không có chi phí transfer dữ liệu CPU-GPU
    \item Dễ tích hợp với code hiện có
\end{itemize}

\textbf{Hạn chế:}
\begin{itemize}
    \item Yêu cầu CPU hỗ trợ AVX2 (không phải CPU nào cũng có)
    \item Code phức tạp hơn, khó debug
    \item Cần xử lý đặc biệt cho các pixel ở biên ảnh
    \item Không tận dụng được sức mạnh của GPU
\end{itemize}
...
\begin{itemize}
    ...
    \item Tính toán thread ID từ block và thread index:
    \begin{lstlisting}
...
\subsubsection{So sánh SIMD và CUDA}

\begin{table}[H]
\centering
\caption{So sánh SIMD và CUDA cho Sobel filter}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Tiêu chí} & \textbf{SIMD (AVX-256)} & \textbf{CUDA} \\
\hline
Tốc độ (speedup) & 3.79x & 14.47x \\
\hline
Yêu cầu phần cứng & CPU AVX2 & GPU NVIDIA \\
\hline
Overhead & Thấp & Memory transfer \\
\hline
Độ phức tạp code & Trung bình & Cao \\
\hline
Portability & Tốt & Chỉ NVIDIA \\
\hline
Phù hợp & CPU-only systems & High-performance systems \\
\hline
\end{tabular}
\end{table}
...
\subsection{Cơ chế fallback trong implementation}

Đề tài triển khai Sobel filter với cơ chế fallback thông minh, tự động chọn phương pháp tối ưu nhất dựa trên phần cứng có sẵn:

\textbf{Thứ tự ưu tiên:}
\begin{enumerate}
    \item \textbf{CUDA (GPU):} Nếu được build với macro \texttt{USE\_CUDA\_SOBEL} và có GPU NVIDIA
    \item \textbf{SIMD với AVX-256:} Fallback nếu CUDA không khả dụng hoặc không thành công
    \item \textbf{CPU OpenMP scalar:} Fallback cuối cùng nếu SIMD không thành công (CPU không hỗ trợ AVX2)
\end{enumerate}

\textbf{Logic trong code:}
...
if (!processed && !sobelSIMD(gray, pkt.sobel)) {
    sobelCPU(gray, pkt.sobel);  // Fallback cuối cùng
}
\end{lstlisting}

\textbf{Ý nghĩa:}
\begin{itemize}
    \item Hệ thống tự động tận dụng phần cứng tốt nhất có sẵn
    \item Đảm bảo hoạt động trên mọi hệ thống (có GPU, không có GPU, CPU cũ)
    \item Người dùng không cần cấu hình thủ công
    \item Performance tự động tối ưu theo từng môi trường
\end{itemize}
...
\subsection{Implementation chi tiết: SIMD với AVX-256}

\subsubsection{Cấu trúc code}

Implementation SIMD trong đề tài sử dụng kết hợp OpenMP và AVX-256 intrinsics:
...
bool sobelSIMD(const cv::Mat& src, cv::Mat& dst) {
    ...
    for (int y = 1; y < rows - 1; ++y) {
        ...
        for (; x <= cols - 9; x += 8) {
            // Load và convert pixels
            // ... AVX intrinsics ...
        }
        
        // Scalar fallback cho pixels còn lại
        for (; x < cols - 1; ++x) {
            // Scalar code
        }
    }
...
\subsubsection{Chi tiết AVX intrinsics}

\textbf{Load và convert dữ liệu:}
\begin{itemize}
    \item Load 16 pixels (128-bit) từ mỗi dòng: \texttt{\_mm\_loadu\_si128}
    \item Convert từ 8-bit unsigned char sang 16-bit integer: \texttt{\_mm256\_cvtepu8\_epi16}
    \item Extract 128-bit từ 256-bit register: \texttt{\_mm256\_extracti128\_si256}
    \item Convert từ 16-bit sang 32-bit integer: \texttt{\_mm256\_cvtepi16\_epi32}
    \item Convert từ 32-bit integer sang float: \texttt{\_mm256\_cvtepi32\_ps}
\end{itemize}

\textbf{Tính toán gradient:}
\begin{itemize}
    \item Tạo constant vector: \texttt{\_mm256\_set1\_ps(2.0f)} để nhân với hệ số 2
    \item Phép nhân vector: \texttt{\_mm256\_mul\_ps} để nhân 8 giá trị float cùng lúc
    \item Phép cộng vector: \texttt{\_mm256\_add\_ps} để cộng 8 giá trị
    \item Phép trừ vector: \texttt{\_mm256\_sub\_ps} để trừ 8 giá trị
\end{itemize}

\textbf{Tính magnitude và store:}
\begin{itemize}
    \item Tính căn bậc hai: \texttt{\_mm256\_sqrt\_ps} cho 8 giá trị float
    \item Clamp giá trị: \texttt{\_mm256\_min\_ps(mag, \_mm256\_set1\_ps(255.0f))}
    \item Convert từ float về integer: \texttt{\_mm256\_cvtps\_epi32}
    \item Pack từ 32-bit về 16-bit: \texttt{\_mm\_packus\_epi32}
    \item Pack từ 16-bit về 8-bit: \texttt{\_mm\_packus\_epi16}
    \item Store 8 pixels: \texttt{\_mm\_store\_epi64}
\end{itemize}

\subsubsection{Xử lý biên ảnh}

Do kernel Sobel 3x3 yêu cầu các pixel lân cận, các pixel ở biên ảnh (dòng đầu, dòng cuối, cột đầu, cột cuối) không thể xử lý bằng SIMD. Có hai cách xử lý:

\begin{enumerate}
    \item \textbf{Bỏ qua biên:} Để giá trị 0 (đã được khởi tạo bằng \texttt{cv::Mat::zeros})
    \item \textbf{Scalar fallback:} Xử lý các pixel còn lại ở cuối mỗi dòng bằng code scalar
\end{enumerate}

Trong implementation, code sử dụng cả hai cách: bỏ qua biên dọc và xử lý scalar cho các pixel còn lại ở cuối mỗi dòng.

\subsection{Implementation chi tiết: CUDA Kernel}

\subsubsection{Kernel function}

CUDA kernel được định nghĩa với modifier \texttt{\_\_global\_\_} để có thể được gọi từ CPU:

\begin{lstlisting}[language=C++]
__global__ void sobelKernel(const unsigned char* src, 
                             unsigned char* dst,
...
Code trên CPU (host) quản lý memory và launch kernel:
...
bool sobelCuda(const cv::Mat& srcGray, cv::Mat& dstEdge) {
    ...
    // Copy từ CPU sang GPU
    cudaMemcpy(d_src, srcGray.data, bytes, 
    ...
    // Launch kernel
    sobelKernel<<<grid, block>>>(d_src, d_dst, 
                                  width, height, 
                                  width, width);
    
    // Synchronize và copy kết quả về
    cudaDeviceSynchronize();
    cudaMemcpy(dstEdge.data, d_dst, bytes, 
...
\begin{itemize}
    ...
    \item Thread ID được tính toán từ 2 chiều:
    \begin{lstlisting}[language=C++]
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    \end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{Grid size:} Tính toán để bao phủ toàn bộ ảnh
    \begin{lstlisting}[language=C++]
    dim3 block(16, 16);  // 256 threads per block
    dim3 grid((width + block.x - 1) / block.x,
              (height + block.y - 1) / block.y);
...
\begin{itemize}
    \item Các threads ở biên ảnh hoặc ngoài phạm vi sẽ tự động return:
    \begin{lstlisting}[language=C++]
    if (x <= 0 || y <= 0 || x >= width - 1 || y >= height - 1) {
        return;  // Thread không làm gì, kết quả = 0
    }
    \end{lstlisting}
...
\textbf{8. Hạn chế và cơ hội cải thiện:}
\begin{itemize}
    \item \textbf{Không dùng shared memory:} Có thể cache các dòng ảnh được dùng nhiều lần
    \item \textbf{Memory access chưa tối ưu:} Có thể cải thiện bằng shared memory để giảm global memory access
    \item \textbf{Boundary threads:} Một số threads không làm việc (overhead nhỏ)
    \item \textbf{Có thể tối ưu thêm:} Sử dụng texture memory hoặc constant memory cho một số trường hợp
\end{itemize}
...
\hline
CPU SIMD & 1.885 ms & 3.79x & 530.4 & SIMD + OpenMP \\
\hline
...
\textbf{CPU Scalar vs SIMD:}
\begin{itemize}
    \item SIMD nhanh hơn 3.79x nhờ xử lý 8 pixels cùng lúc
    \item Overhead của SIMD intrinsics là tối thiểu
    \item Kết hợp với OpenMP cho parallelism theo dòng
\end{itemize}

\textbf{SIMD vs CUDA:}
\begin{itemize}
    \item CUDA nhanh hơn SIMD 3.82x
    \item GPU có hàng nghìn lõi vs CPU có vài lõi
    \item CUDA có overhead memory transfer nhưng vẫn nhanh hơn nhiều
\end{itemize}

\textbf{Khi nào dùng SIMD, khi nào dùng CUDA:}
\begin{itemize}
    \item \textbf{SIMD:} Khi không có GPU, hoặc overhead transfer lớn hơn lợi ích
    \item \textbf{CUDA:} Khi có GPU và cần tốc độ tối đa, xử lý batch lớn
\end{itemize}
...
\textbf{Cấu trúc TSQueue:}
\begin{lstlisting}[language=C++]
template<typename T>
class TSQueue {
    std::mutex mutex_;                    // Mutex để đồng bộ
    std::condition_variable cond_;        // Condition variable
    std::queue<T> queue_;                 // Queue chứa dữ liệu
    size_t maxSize_;                      // Kích thước tối đa (5 frames)
    
    void push(const T& value);            // Thêm vào queue
    bool pop(T& out);                     // Lấy ra khỏi queue
...
\begin{itemize}
    \item \textbf{Nhiệm vụ:} Áp dụng bộ lọc Sobel với tối ưu song song (CUDA/SIMD/CPU)
    ...
    \begin{itemize}
        ...
        \item Sử dụng cơ chế fallback: CUDA → SIMD → CPU scalar
...
\textbf{Trước (Detection + OCR cùng thread):}
\begin{verbatim}
Thread 1: Capture
Thread 2: Sobel
Thread 3: Detect + OCR (tuần tự)  ← Chậm
Thread 4: Render
\end{verbatim}
\begin{itemize}
...
\textbf{Sau (Detection và OCR tách riêng):}
\begin{verbatim}
Thread 1: Capture
Thread 2: Sobel
Thread 3: Detection (chỉ detect)   ← Độc lập
Thread 4: OCR (chỉ OCR)            ← Độc lập
Thread 5: Render
\end{verbatim}
\begin{itemize}
...
\begin{itemize}
    \item Mỗi thread chạy trong vòng lặp \texttt{while} với điều kiện:
    \begin{lstlisting}[language=C++]
    while (running_.load() || !qInput_.empty()) {
        // Xử lý frame
    }
    \end{lstlisting}
