...
\subsection{Kết quả nhận dạng biển số}

Hệ thống đã được kiểm thử trên ảnh thực tế và cho kết quả nhận dạng chính xác. Dưới đây là ví dụ kết quả nhận dạng biển số xe máy:

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Image/Result.png}
\caption{Kết quả nhận dạng biển số xe máy: Hệ thống phát hiện và nhận dạng thành công biển số "29D2-49969" từ ảnh đầu vào}
\label{fig:result}
\end{figure}

Như có thể thấy trong hình \ref{fig:result}, hệ thống đã:
\begin{itemize}
    \item Phát hiện chính xác vị trí biển số xe máy màu đỏ (bounding box màu đỏ)
    \item Nhận dạng thành công số biển số: \textbf{29D2-49969}
    \item Hiển thị kết quả OCR trên ảnh (text màu xanh lá)
    \item Xử lý được trong điều kiện thực tế với nhiều đối tượng trong khung hình
    \item Hiển thị FPS = 21 ở góc trên bên trái (số 21 màu xanh lá)
\end{itemize}

\subsubsection{Phân tích FPS: Sobel Filter vs Toàn Bộ Pipeline}

Có sự khác biệt đáng kể giữa FPS của Sobel filter riêng lẻ và FPS của toàn bộ pipeline:

\textbf{FPS của Sobel Filter (2025.5 FPS):}
\begin{itemize}
    \item Đây là FPS khi chỉ đo riêng bước xử lý Sobel filter với CUDA
    \item Chỉ tính thời gian xử lý Sobel: ~0.494 ms/frame
    \item Không bao gồm các giai đoạn khác của pipeline
    \item Đây là benchmark để so sánh hiệu năng của các phương pháp song song hóa Sobel
\end{itemize}

\textbf{FPS của Toàn Bộ Pipeline (21 FPS):}
\begin{itemize}
    \item Đây là FPS end-to-end của toàn bộ hệ thống từ Capture đến Render
    \item Bao gồm tất cả các giai đoạn:
    \begin{itemize}
        \item \textbf{Capture:} Đọc frame từ camera/file (~1-5 ms)
        \item \textbf{Sobel:} Xử lý Sobel với CUDA (~0.5 ms) ← Rất nhanh
        \item \textbf{Detection:} Chạy YOLOv5 model (~30-50 ms) ← Bottleneck chính
        \item \textbf{OCR:} Chạy OCR model (~20-40 ms) ← Bottleneck thứ hai
        \item \textbf{Render:} Vẽ kết quả và hiển thị (~5-10 ms)
    \end{itemize}
    \item Tổng latency: ~50-100 ms/frame → FPS = 10-20 FPS
    \item FPS 21 trong hình là kết quả thực tế của pipeline hoàn chỉnh
\end{itemize}

\textbf{Tại sao có sự khác biệt lớn?}
\begin{itemize}
    \item \textbf{Bottleneck không phải Sobel:} Sobel filter chỉ chiếm ~0.5 ms trong tổng ~50 ms
    \item \textbf{Detection và OCR là bottleneck:} Hai giai đoạn này chiếm ~70-90\% tổng thời gian xử lý
    \item \textbf{Model inference chậm:} YOLOv5 models (detection và OCR) cần thời gian inference đáng kể, đặc biệt khi chạy trên CPU
    \item \textbf{Pipeline parallelism giúp:} Nhờ pipeline với 5 threads, các frame được xử lý chồng chéo, giúp tăng throughput từ ~10 FPS (sequential) lên ~21 FPS
\end{itemize}

\textbf{Kết luận:}
\begin{itemize}
    \item Sobel filter với CUDA đạt 2025.5 FPS chứng tỏ tối ưu hóa song song hóa thành công
    \item FPS 21 của pipeline là hợp lý vì bottleneck nằm ở Detection và OCR models
    \item Nếu muốn tăng FPS pipeline, cần tối ưu Detection và OCR (ví dụ: chạy trên GPU, quantization, batch processing)
    \item Sobel filter không phải là bottleneck, nên việc tối ưu nó đã đạt mục tiêu
\end{itemize}

Kết quả này chứng tỏ pipeline xử lý với Sobel filter và CUDA đã hoạt động hiệu quả. Sobel filter được tối ưu tốt (2025.5 FPS) và không phải là bottleneck của hệ thống. FPS 21 của pipeline là do các giai đoạn Detection và OCR chiếm phần lớn thời gian xử lý.
