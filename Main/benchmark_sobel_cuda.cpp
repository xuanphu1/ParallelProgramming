...
int main(int argc, char** argv) {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "BENCHMARK SOBEL FILTER" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    hasCuda = true;
    std::cout << "[INFO] CUDA enabled" << std::endl;
    // Test CUDA có hoạt động không
    if (sobelCuda(img, cudaResult)) {
        std::cout << "[INFO] CUDA test: OK" << std::endl;
    } else {
        std::cout << "[WARNING] CUDA test: Failed, fallback to CPU" << std::endl;
        hasCuda = false;
    }
    #else
    std::cout << "[INFO] CUDA disabled (build without CUDA)" << std::endl;
    #endif
    ...
    double time_cuda = 0;
    if (hasCuda) {
        time_cuda = benchmark_sobel_impl("GPU CUDA", sobelCuda, img);
    } else {
        std::cout << "  " << std::left << std::setw(25) << "GPU CUDA" << ": " 
                  << "Khong co (build khong co CUDA)" << std::endl;
    }
    ...
    if (hasCuda && time_cuda > 0) {
        std::cout << "GPU CUDA:            " << std::setw(8) << std::setprecision(3) << time_cuda << " ms (x" 
                  << std::setprecision(2) << (time_cpu / time_cuda) << " nhanh hon)" << std::endl;
        std::cout << std::endl;
        std::cout << "So sanh chi tiet:" << std::endl;
        std::cout << "  CUDA vs CPU scalar: " << std::setprecision(2) << (time_cpu / time_cuda) << "x" << std::endl;
        std::cout << "  CUDA vs SIMD:       " << (time_simd / time_cuda) << "x" << std::endl;
    }
    ...
    if (hasCuda && time_cuda > 0) {
        out << "  GPU CUDA:            " << std::setprecision(3) << time_cuda << " ms (speedup: " 
            << std::setprecision(2) << (time_cpu / time_cuda) << "x)\n";
        out << "\n";
        out << "Detailed comparison:\n";
        out << "  CUDA vs CPU scalar: " << std::setprecision(2) << (time_cpu / time_cuda) << "x\n";
        out << "  CUDA vs SIMD:       " << (time_simd / time_cuda) << "x\n";
    }
    out.close();
